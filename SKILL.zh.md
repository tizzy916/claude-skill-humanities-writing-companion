...
**本 skill 端到端**:覆盖一篇人文论文的完整生命周期——从研究问题 sharpening(Mode H)、文献脉络梳理(Mode I)、规划专项(Mode J)、起草(Mode C/A)、四层章节审读(Mode B)、可校准对抗性审稿(Mode D)、写作瓶颈辅助(Mode E)、底稿修订含 revision-coach(Mode F)、盲读核对(Mode G),直到投稿前 AI 使用披露(Mode K)——外加 `scripts/` 下的引用工具链(一致性、格式转换、Crossref 验证)。
...
| **工作模式** | A-K 11 种模式覆盖写作生命周期(H/I/J 写作前、C/A 起草、B/D 审读、E/F 修订、G/K 投稿前) |
...
**学科特化的方法论攻击**(读取 `_writing-config/学科档案.md` 知道部署哪些):

| 学科 | 方法论攻击 |
|---|---|
| **历史** | 史料处理:一手 / 二手区分是否维持?史料偏倚是否被考虑?时代错置:现代范畴是否被悄悄投射到历史行动者身上?反事实:你能接受持相反立场的人用这种论证方法吗? |
| **哲学** | 论证形式:这是一个有显式前提的形式论证,还是被装扮成形式的实质推理?概念使用:你用 "X" 是技术意义上还是日常意义上?模态范围:你说"必然"是逻辑、形而上学、还是法则意义?是否在偷换? |
| **文学** | 文本锚定:每个解释性主张是否锚定在文本证据上?诠释学循环:你的解释是否被预先带入的理论框架决定?文类意识:你是逆文类惯例读这个文本,还是顺着读? |
| **文化研究** | 位置性:你自己的位置是被承认还是被隐藏?权力/知识:谁在说话,谁被说?概括范围:这个案例表明 X——它在这个场所、时期、人群表明 X,还是普遍表明 X? |
| **艺术史** | 描述和解释是否分开?来源证据是否引用?接受史:作品在其当下是否被这样读,还是这只是当代接受?物质主张和图像志主张是否区分? |
| **宗教学 / 古典学** | 源语言:你在读原文,还是依赖译本?传统位置:你预设了哪个传统的解读?内部 / 外部:emic 主张与 etic 主张是否清楚区分? |
| **跨学科案例** | 为什么这个案例(典型、例外、偏离案例)?案例-叙事 vs 案例-论证——你是否在用叙事逃避论证工作? |

**输出标记**:方法论专项的攻击在 `_meta/交互记录.md` 中标记为 `[Mode D · 方法论]`,以便作者回应时区分方法论问题与内容问题。
...
3. **应用学科特化弧线**。每个主学科有一个反复出现的修辞结构。用以下作为起点模板(作者可偏离,但偏离应是选择而非疏漏):

   | 学科 | 标准弧线 |
   |---|---|
   | **哲学** | 概念-puzzle → 概念分析 → 应对最强反对 → 对下游争论的影响 |
   | **历史** | 史学史定位 → 叙事 → 分析性论证 → 更广泛意义 / 重新分期 |
   | **文学** | 理论框架 → 文本细读 → 回到框架的概括 / 通过文本重读框架 |
   | **文化研究** | 案例 + 理论镜头 → 分析展开 → 对分析本身的反思性回转 |
   | **艺术史** | 描述 → 语境化(来源、接受)→ 解释 → 对观看史的影响 |
   | **宗教学 / 古典学** | 文本-语文学工作 → 传统定位 → 解释性论证 → 对当代学术的影响 |
