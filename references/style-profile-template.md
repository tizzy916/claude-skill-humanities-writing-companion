...
> This profile is the "constitution" of the author's voice — the first anchor file in the Anti-Drift Protocol, a precondition for Mode F (draft revision), and required reading before every writing assistance (Mode A included). It pairs with the target reader profile:
> - The reader profile tells the AI "whom the author is speaking to"
> - This profile tells it "how the author speaks"
>
> **How it is built**: generated by analyzing 2–3 of the author's earlier pieces that were never touched by AI (pre-AI samples are the only uncontaminated baseline), then continuously updated through dialogue observation and revision feedback.
> The humanities-writing-companion skill copies this template into the project's
> `_writing-config/style-profile.md` (Chinese path: `_writing-config/写作风格档案.md`) during onboarding.
> **A blank slot still beats having no profile** — but fill the surface-features section from real samples before the first drafting session; do not guess it.
