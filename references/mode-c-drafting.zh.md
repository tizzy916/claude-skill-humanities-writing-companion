...
当用户带着一个初步想法（而非已有草稿）来时，按顺序运行写作前模式——这个场景是 Mode H→I→J→C 的编排，不是一条独立流水线：

1. **先初始化项目**：按 project-management.md 创建文件夹结构、写作风格档案（首次需要作者提供 1-2 篇已有文字作为风格样本）——后续模式的产物要写进这里
2. **Mode H**：把模糊兴趣锐化为研究问题（`研究问题.md`）
3. **Mode I**：梳理作者实际读过的文献（`文献地图.md`）。作者读得还不够时，Mode I 会明说——阅读是作者自己的工作；只建议方向性主题，绝不凭记忆报具体书目
4. **Mode J**：规划结构（`论文大纲.md`）——苏格拉底式：结构由作者自己发现，AI 不递交现成结构
